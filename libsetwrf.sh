#!/bin/bash
#Written by Jing Xiang CHUNG on 11/10/2024
...
tar -xzvf ${PATH_to_SRC}/jpegsrc.v9f.tar.gz; cd jpeg-9f;./configure --prefix=${INSTALL_DIR};make;make install;cd ..
...
echo "...compiling hdf5..."
tar -xzvf ${PATH_to_SRC}/hdf5-1.14.4-3.tar.gz; cd hdf5-1.14.4-3;./configure --prefix=${INSTALL_DIR} --enable-hl --enable-build-mode=production --with-pic --with-szlib=${INSTALL_DIR} --enable-threadsafe --enable-unsupported;make;make install;cd ..

#5. NetCDF4(need HDF5)---------------------------
#netcdf-4.9.2: https://downloads.unidata.ucar.edu/netcdf/
echo "...compiling netcdf-c..."
tar -xzvf ${PATH_to_SRC}/netcdf-c-4.9.2.tar.gz; cd netcdf-c-4.9.2;./configure --prefix=${INSTALL_DIR} --enable-shared --enable-netcdf-4;make;make install;cd ..
